...
#         if full_text.strip():
#             print(f"✅ Extracted {len(pages)} pages via pypdf")
#             # 🔍 DEBUG — اطبعي أول 500 حرف من النص المستخرج عشان تتأكدي إنه فعلاً
#             # بيحتوي على بيانات حقيقية مش فاضي
#             print("──── DEBUG: first 500 chars of extracted text ────")
#             print(full_text[:500])
#             print("────────────────────────────────────────────────")
#             return full_text
#         # لو مفيش text → جرب docling
#         print("⚠️  pypdf got empty text, falling back to docling...")
#         from docling.document_converter import DocumentConverter
#         converter = DocumentConverter()
#         result = converter.convert(file_path)
#         md = result.document.export_to_markdown()
#         print("──── DEBUG: first 500 chars (docling) ────")
#         print(md[:500])
#         print("────────────────────────────────────────────")
#         return md
#     except Exception as e:
...
# 🛑 RULE 0 — BLANK CV GUARDRAIL (CHECK THIS FIRST, ALWAYS)
# ═══════════════════════════════════════════════════════════
# Before doing ANYTHING else, scan the CV for actual numeric or descriptive values related to
# the player's profile (e.g., age, speed/pace, scoring output, technical skills, appearances/matches,
# or equivalent metrics — regardless of the exact field name or language used in the CV).

# Only output the blank-template message below if the document is GENUINELY EMPTY or contains
# ONLY placeholder text (e.g., "___", "N/A", "TBD", "Lorem ipsum", or literally no player data at all).
# If the CV has REAL data under different labels, abbreviations, a different language, or a different
# layout/table structure — that is NOT a blank CV. Proceed with the analysis using whatever data is present,
# and if a specific metric is missing, simply state "data not provided" for that one item only —
# do NOT reject the entire CV.

# If after this careful check the document truly has no usable player data at all, output EXACTLY:

# "❌ INSUFFICIENT DATA: The provided CV appears to be a blank template. Please provide a CV with actual player metrics, statistics, and history so I can give you a proper evaluation."

# Do NOT invent data. Do NOT hallucinate statistics.
...
def analyze_athletic_profile(parsed_text: str, specific_question: str = None) -> str:
    """
    ملحوظة: الدالة دلوقتي بتستقبل parsed_text جاهز (مش file_path) عشان تشتغل
    سواء كان النص جايّ من ملف جديد اتقرا دلوقتي، أو من cv_text المخزّن في الـ state
    من رفعة سابقة في نفس المحادثة.
    """
    if parsed_text.startswith("Error"):
        return parsed_text
    client = Groq(api_key=settings.GROQ_API_KEY)
...
def answer_followup_question(parsed_text: str, question: str) -> str:
    """
    بتستخدم لما يكون فيه CV متخزن بالفعل (مش ملف جديد) والمستخدم بيسأل سؤال متابعة
    محدد (زي 'إيه أفضل تمرين ليه؟' أو 'وضّح أكتر النقطة التالتة').
    على عكس analyze_athletic_profile، الدالة دي بترجع إجابة مركّزة على السؤال بس
    من غير ما تعيد التقرير الكامل بالـ 6 sections.
    """
    client = Groq(api_key=settings.GROQ_API_KEY)
    system_prompt = """
You are an Elite Athletic Scout, Tactical Analyst, and High-Performance Career Coach.
You previously analyzed this player's CV in full. The player is now asking a FOLLOW-UP
question about that same CV/report.

RULES:
- Answer ONLY the specific follow-up question asked. Do NOT repeat the full 6-section report.
- Do NOT regenerate the greeting, the Positional Alignment, Elite Strength, Critical
  Vulnerability, Career Strategy, or Next Steps sections unless the question specifically
  asks about that exact topic.
- Base your answer on the CV data provided below — cite real stats/values when relevant.
- Keep the same expert tone (scout / tactical analyst / performance coach), but keep the
  answer concise and directly useful — a few sentences to a short paragraph, plus specific
  actionable detail (drill names, rep ranges, league names, etc.) ONLY if relevant to the question.
- Do NOT invent data that isn't in the CV. If something needed to answer isn't in the CV,
  say so plainly instead of guessing.
"""
    user_content = (
        f"Here is the player's CV data (already analyzed once):\n\n{parsed_text}\n\n"
        f"Follow-up question from the player:\n{question}"
    )
    chat_completion = client.chat.completions.create(
        model="openai/gpt-oss-120b",
        messages=[
...
def feedback_node(state: GraphState) -> dict:
    ...
    try:
        file_path = state.get("file_path", "")
        question = state.get("user_input", "")
        stored_cv_text = state.get("cv_text", "")

        if file_path:
            # ── حالة: فيه ملف جديد اتبعت دلوقتي → نستخرج النص ونحدّث الـ state ──
            print("   📎 New file received — extracting text...")
            parsed_text = extract_portfolio_text(file_path)
            if parsed_text.startswith("Error"):
                return {
                    "feedback_output": "",
            ...
            answer = analyze_athletic_profile(parsed_text=parsed_text, specific_question=question)
            return {
                "feedback_output": answer,
                "final_answer": answer,
                "cv_text": parsed_text,  # نخزّن النص عشان أسئلة المتابعة الجاية
                "chat_history": [AIMessage(content=answer)],
            }

        elif stored_cv_text:
            # ── حالة: follow-up question، مفيش ملف جديد، بس عندنا نص محفوظ من قبل ──
            # هنا بنستخدم answer_followup_question (مش analyze_athletic_profile)
            # عشان نرد على السؤال المحدد بس، من غير ما نكرر التقرير الكامل بالـ 6 sections.
            print("   💾 No new file — using cv_text stored in state (follow-up question)")
            answer = answer_followup_question(parsed_text=stored_cv_text, question=question)
            return {
                "feedback_output": answer,
                "final_answer": answer,
                "chat_history": [AIMessage(content=answer)],
            ...
            # ── حالة: مفيش ملف جديد ولا نص محفوظ → فعلاً مفيش CV نشتغل عليه ──
            answer = "❌ No file provided. Please upload your CV or athletic profile PDF."
            return {
                "feedback_output": "",
                "final_answer": answer,
