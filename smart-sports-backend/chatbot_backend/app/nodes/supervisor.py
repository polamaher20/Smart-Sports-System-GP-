...
# ── File type → agent (deterministic, skips the LLM entirely) ────
_FILE_TYPE_ROUTE = {
    "video":    "player_analysis",
    "document": "feedback",
}
...
def supervisor_node(state: GraphState) -> dict:
    ...
    # ── Deterministic routing when the file type is known ────────
    # Skips the LLM call entirely — more reliable, faster, cheaper.
    if file_type in _FILE_TYPE_ROUTE:
        route = _FILE_TYPE_ROUTE[file_type]
        print(f"   ⚡ Deterministic route (file_type='{file_type}') → {route.upper()}")
        print(f"{'━'*60}")
        return {
            "route":        route,
            "chat_history": [HumanMessage(content=state["user_input"])],
    ...
    # ── Otherwise, fall back to the LLM (no file, or unknown type) ─
    chain = _supervisor_prompt | _supervisor_llm
    result = chain.invoke({
        "user_input":   state["user_input"],
        "has_file":     state.get("has_file", False),
